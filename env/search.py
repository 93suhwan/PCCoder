...
import params
from env.statement import index_to_statement, statement_to_index, Statement
...
def dfs_repair(env, max_depth, model, width, timeout, userProgram, threshold):
    start_time = time.time()
    state = {'num_steps': 0, 'num_invalid': 0}
    state['end_time'] = start_time + timeout
...
def cab_repair(env, max_depth, model, beam_size, width, width_growth, timeout, userProgram, threshold, max_beam_size=6553600):
    start_time = time.time()
    state = {'num_invalid': 0, 'num_steps': 0, 'end_time': start_time + timeout}

    res = False
    while time.time() < state['end_time']:
        res = beam_search_repair(env, max_depth, model, beam_size, width, state, userProgram, threshold)
        if res is not False or beam_size >= max_beam_size:
            break
        beam_size *= 2
        width += width_growth
...
def beam_search_repair(env, max_depth, model, beam_size, expansion_size, state, userProgram, threshold):
    def helper(beams, state):
        if time.time() >= state['end_time']:
            return FAILED_SOLUTION

        for env, statements, _, _ in beams:
            if env.is_solution():
                return statements
        ...
        for beam_num, (env, statements, prob, userProgs) in enumerate(beams):
            if time.time() >= state['end_time']:
                return FAILED_SOLUTION
            copiedUserProgram = copy.deepcopy(userProgs)

            if env.num_vars < params.max_program_vars:
                to_drop = None
            else:
                to_drop = drop_indx[beam_num]
                currIdx = len(env.states[0].vars)
                for i in range(currIdx - len(copiedUserProgram.input_types) + 1, len(copiedUserProgram.statements)):
                    f, args = copiedUserProgram.statements[i].function, list(copiedUserProgram.statements[i].args)
                    for j, arg in enumerate(args):
                        if isinstance(arg, int):
                            if arg == currIdx:
                                ...
                                args[j] -= 1
                    copiedUserProgram.statements[i] = Statement(f, args)
            ...
            for number, statement_index in enumerate(reversed(statement_pred[beam_num, -expansion_size:])):
                ...
                state['num_steps'] += 1
                copiedUserProgram._encoded = None
                new_beams.append((new_env, statements + [env.statement_to_real_idxs(statement)], prob * statement_probs[beam_num, statement_index], copiedUserProgram))
    ...
    res = helper([(env, [], 1, userProgram)], state)
    if res == FAILED_SOLUTION:
        res = False
